...
uniform_int_distribution<> Uniint(0,600);
uniform_int_distribution<> backoff(0,Backoff_Indicator);

//反応率計算
double calpr(int waittime, double alpha, double u0){
  double a,b;
...
/*
    if(t == 2504){
      cout << "t = " << t << endl;
      for(i = 0; i < N; i++){
        cout << waitflag[2504/S][iot[i].locate] << ", ";
      }
      cout << endl;
      cout << "finish" << endl;
      for (l = 0 ; l < 144 ; l++){
...
*/
        ...
        if(iot[i].active[k] == true){
          if(iot[i].traffic_list[k][0].starttime2 <= t){
            ...
            if(waitflag[t/S][iot[i].locate] == 1){
              if(calpr(waittime, alpha[k], u0[k]) > Uniform(mt3) && iot[i].traffic_list[k][0].wait == false){
  ...
  for(k = 0 ; k < 4 ; k++){
    filename = "./result/delay"+to_string(k)+"_wait"+to_string(r)+".csv";
    fout.open(filename);
